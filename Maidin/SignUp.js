...
const SignUp = ({ route, navigation }) => {
    ...
    const submit = async () => {
        ...
        if(isFirstNameValid && isLastNameValid && isCNICValid && isExperienceValid && isExperienceValid && isEmailValid && isPasswordValid && isConfirmedValid && password_match && filePath && serviceType){
            ...
            //     console.log(err)
            // })
            await axios.post('http://192.168.100.131:3000/RegistrationCheck',{
                cnic : cnic,
                contact : contact,
                email : email
            }).then((response)=>{
                if(response.data.exists){
                    Alert.alert(
                        'Maid-In',
                        'Person with same CNIC,Contact or Email Already exists in the System'
                    )
                }
        ...
        <ScrollView style = {styles.container}>
            ...
            <View style = {styles.footer}>
                <View style = {styles.footerContent}>
                    ...
                    style = {{color : '#36FF92' , fontWeight : 'bold',marginLeft : 5}}>
                      Sign In
                    </Text>
                </View>
            </View>
